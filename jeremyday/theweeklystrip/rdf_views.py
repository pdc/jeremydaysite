...
def strip_conneg(request, number):
    ...
    for accept_clause in accept_raw.split(","):
        ...
        params = dict(
            (k.strip(), v.strip()) for (k, v) in ((kv.split("=", 1) for kv in terms))
        )
        ...
        if best_q is None or q > best_q:
            for media in options.keys():
                if (
                    media_range == "*/*"
                    or media_range.endswith("/*")
                    and media.startswith(media_range[:-1])
...
def strip_rdf(request, number, format):
    ...
    response = HttpResponse(
        graph.serialize(format=format), content_type=format_content_types[format]
    )
    return response
