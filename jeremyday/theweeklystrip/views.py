...
def render_with_template(
    ...
    def decorator(func):
        def wrapped_handler(
            ...
            return render(
                request,
                template_name or default_template_name,
                result,
...
def strip_page(request, number):
    ...
    if ordinal > len(strips):
        return HttpResponseRedirect(
            reverse("tws_strip", kwargs={number: str(len(strips))})
        )
    ...
    twitter_share_params = [
        (
            ...
            request.build_absolute_uri(
                reverse("tws_strip", kwargs={"number": "%d" % strip["number"]})
            ),
        ),
        ("via", "cleanskies"),
        (
            "text",
            "Jeremy Day’s The Weekly Strip {0} ‘{1}’".format(
...
def by_date(request, year, month, day):
    ...
    if hi < len(strips):
        return HttpResponseRedirect(
            reverse("tws_strip", kwargs={"number": strips[hi]["number"]})
        )
    raise Http404
...
def reading_order_feed(request, page=None):
    ...
    for strip in subset:
        ...
        strip["page_href"] = request.build_absolute_uri(
            reverse("tws_strip", kwargs={"number": str(strip["number"])})
        )
        updated = time.strftime("%Y-%m-%dT%H:%M:%S%z", time.localtime(strip["mtime"]))
        updated = "%s:%s" % (
            updated[:-2],
            updated[-2:],
        )  # RFC 3339 required +00:00 not +0000
        ...
        if (
            not feed_updated or updated > feed_updated
        ):  # This is the first time I have actually exploited the sortability of RFC 3339 datetimes!
